...
function App() {
    ...
    const API_URL = window.config?.API_URL || 'http://localhost:8000'; // Matched with local_server.py
    ...
    const handleUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            ...
            // Poll for completion (Wait for S3 Trigger -> Lambda -> DynamoDB)
            setIsUploading(true); // Keep spinner on during polling
            let attempts = 0;
            const pollInterval = setInterval(async () => {
                attempts++;
                await fetchDocuments();

                // Stop polling after 10 attempts (20 seconds) or let user see list update
                if (attempts >= 10) {
            ...
            // Stop spinner after 4 seconds (Optimistic UX)
            setTimeout(() => {
                setIsUploading(false);
