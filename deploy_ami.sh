...
# 0. Check Dependencies (New Laptop / CloudShell Fix)
if ! command -v terraform &> /dev/null; then
    echo -e "${BLUE}[0/5] Terraform not found. Installing for CloudShell...${NC}"
    if [ -f /etc/system-release ] && grep -q "Amazon Linux" /etc/system-release; then
        ...
        echo -e "${BLUE}Warning: Not on Amazon Linux. Please install Terraform manually if this fails.${NC}"
    fi
fi
